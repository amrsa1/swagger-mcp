...
function findMcpConfig() {
  ...
  for (const configPath of possiblePaths) {
    ...
    if (fs.existsSync(fullPath)) {
      log.info(`Found MCP config at: ${configPath}`);
      return fullPath;
    }
  }
...
try {
  ...
  if (mcpPath) {
    ...
    const apiConfig = mcpConfig.servers?.['swagger-mcp']?.env || {};
    ...
    if (API_BASE_URL) {
      configLoadedFrom = path.basename(path.dirname(mcpPath)) + '/' + path.basename(mcpPath);
    }
  }
} catch (error) {
